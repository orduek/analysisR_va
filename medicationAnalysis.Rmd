...
```{r}
# plot age
ggplot(antiDP_dat, aes(x=antiDP_dat$AGE_OCT01)) + geom_histogram() + xlab("Age") + stat_bin(bins=30) + theme_minimal() + ggtitle("Age Histogram")
plot(antiDP_dat$MARRIED)

g <- data.frame(table(antiDP_dat$race))
ggplot(g, aes(x= "",y=Freq, fill = Var1)) + geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) + theme_minimal()
```
Now lets look at the drugs

```{r}
# whats the average drug usage?
drugeUsage<- table(antiDP_dat$ADDAY)#, antiDP_dat$APSDAY, antiDP_dat$AXDAY, vaDatclean$STIMDAY)
mean(antiDP_dat$ADDAY)
sd(antiDP_dat$ADDAY)
median(antiDP_dat$ADDAY)
plot(density(antiDP_dat$ADDAY))
```
...
fit.coxph3 <- coxph(surv_object ~ FEMALE  + VATXM + MARRIED + OEFOIF + race + sumComorbidfactor, 
                   data = antiDP_dat)
