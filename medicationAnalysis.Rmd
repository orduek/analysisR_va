...
ggsurvplot(fit1, data = drugLess30, pval = TRUE)
...
#summary(fit1)
ggsurvplot(fit_psych, data = medicatedVa_dat, pval = TRUE)
```
...
307798/330772
medicatedVa_datFirstV$timeTofV <- factor(case_when(medicatedVa_datFirstV$MHFUFRST<=7 ~ "7", medicatedVa_datFirstV$MHFUFRST > 7 & medicatedVa_datFirstV$MHFUFRST <= 14 ~ "7-14", TRUE ~ "14+"))
...
summary(surv_firstV)
fit.coxphFirstV <- coxph(surv_firstV ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbid + sumMed + timeTofV, 
                   data = medicatedVa_datFirstV)
...
fit.coxph5 <- coxph(surv_objectApx ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbidfactor + sumMedCat, 
                   data = antiAPX_dat)
print(ggforest(fit.coxph5, data = antiAPX_dat, main = "Anti Psychotic"))
```
```{r}
splots = NULL
splots1 <- ggforest(fit.coxph3, data = antiDP_dat)
splots2 <- ggforest(fit.coxph4, data = antiANX_dat)
splots3 <- ggforest(fit.coxph5, data = antiAPX_dat)
pdf("threplots.pdf")
ggarrange(splots1, splots2, splots3, ncol = 1, nrow = 3, labels = c("A-D","A-X","A-P")) #, widths = c(2,2,2))
dev.off()
ggforest(fit.coxph4, data = antiANX_dat, main = "Anxiolytics")
```
