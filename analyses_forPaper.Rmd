...
modelTotal <- glm(threeMedsCensore ~ scale(pclTotal) + AGE_OCT01 + PSYCTHR + MARRIED+ sumComorbid + sumMed, data=pclNoNa, family = binomial)
summary(modelTotal)
...
model3 <- glm(censore30 ~ scale(pclR) + scale(pclA) + scale(pclN) + scale(pclDA) + scale(pclAA) + AGE_OCT01 + PSYCTHR + MARRIED+ sumComorbid + sumMed + race, data=pclNoNa, family = binomial)
summary(model2)
summ(model3, exp = TRUE, scale =TRUE, digits = 5)
plot_model(model3)

model3pclTotal <- glm(censore30 ~pclTotal + AGE_OCT01 + PSYCTHR + MARRIED+ sumComorbid + race + sumMed  , data=pclNoNa, family = binomial)
