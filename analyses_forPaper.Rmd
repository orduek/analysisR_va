...
medicatedVa_dat$antiDPCensore <- case_when(medicatedVa_dat$ADDAY>=180 ~ 0, medicatedVa_dat$ADDAY<180 ~ 1, is.na(medicatedVa_dat$ADDAY) ~ NA_real_, TRUE ~ 3)
summary(medicatedVa_dat$antiDPCensore)
table(medicatedVa_dat$antiDPCensore)
table(medicatedVa_dat$MHDROP)
# build cencor var for anxiolytics
medicatedVa_dat$antiAXCensore <- case_when(medicatedVa_dat$AXDAY>=180 ~ 0, medicatedVa_dat$AXDAY<180 ~ 1, is.na(medicatedVa_dat$AXDAY) ~ NA_real_, TRUE ~ 3)
table(medicatedVa_dat$antiAXCensore)
# build cencor var for anti psychotic
medicatedVa_dat$antiPCensore <- case_when(medicatedVa_dat$APSDAY>=180 ~ 0, medicatedVa_dat$APSDAY<180 ~ 1, is.na(medicatedVa_dat$APSDAY) ~ NA_real_, TRUE ~ 3)
table(medicatedVa_dat$antiPCensore)
...
First -- Anti depression

```{r antiDP_analysis, echo=FALSE}
# now we clean data
# take only anti depressant without any medication before index day
antiDP_dat <- dplyr::filter(medicatedVa_dat, !is.na(ADRXN) & is.na(PREADRX) & is.na(PREAPRX) & is.na(PREAXRX) & is.na(PRESTIRX) & is.na(PREACRX) & is.na(PRELIRX) & is.na(PREOPRX) & is.na(PREPRZRX)) 
head(antiDP_dat)
...
summary(surv_antiDP)
# now cox analysis
fit.coxpDP <- coxph(surv_antiDP ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbid + sumMed, 
                   data = antiDP_dat)
...
antiANX_dat <- dplyr::filter(medicatedVa_dat, !is.na(AXSEDHPRXN) & is.na(PREADRX) & is.na(PREAPRX) & is.na(PREAXRX) & is.na(PRESTIRX) & is.na(PREACRX) & is.na(PRELIRX) & is.na(PREOPRX) & is.na(PREPRZRX)) 
head(antiANX_dat)
...
# now cox analysis
fit.coxpANX <- coxph(surv_antiANX ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbid + sumMed, 
                   data = antiANX_dat)
...
antiAPX_dat <- dplyr::filter(medicatedVa_dat, !is.na(APSRXN) & is.na(PREADRX) & is.na(PREAPRX) & is.na(PREAXRX) & is.na(PRESTIRX) & is.na(PREACRX) & is.na(PRELIRX) & is.na(PREOPRX) & is.na(PREPRZRX)) 

## survival model
surv_antiAP <- Surv(time = antiAPX_dat$APSDAY, event = antiAPX_dat$antiPCensore)  
summary(surv_antiAP)
# now cox analysis
fit.coxpAP <- coxph(surv_antiAP ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbid + sumMed, 
                   data = antiAPX_dat)
...
threeMeds_dat <- dplyr::filter(medicatedVa_dat, !is.na(APSRXN) | !is.na(AXSEDHPRXN) | !is.na(ADRXN) & is.na(PREADRX) & is.na(PREAPRX) & is.na(PREAXRX) & is.na(PRESTIRX) & is.na(PREACRX) & is.na(PRELIRX) & is.na(PREOPRX) & is.na(PREPRZRX))
# build total timevar (last rx - first rx between all those meds)
  # take date of first rx of first med and last rx of last med and calculate number of days
  # create var of first date (that compared antiDP, antiolytic and antipsychotic)
...
# now cox analysis
fit.coxpThreeMeds <- coxph(surv_ThreeMeds ~ PSYCTR + MARRIED + OEFOIF + race + sumComorbid + sumMed, 
                   data = threeMeds_dat)
...
## now we do survival analysis
surv_30days <- Surv(time = threeMeds_30days$totalDaysThreeMeds, event = threeMeds_30days$censore30)  
summary(surv_30days)
# now cox analysis
...
```
```{r binomial_regression}
# use thr 30 days dataset to run binomial regression analysis

model1 <- glm(censore30 ~ AGE_OCT01 + PSYCTR + sumComorbid + sumMed, data=threeMeds_30days, family = binomial)
summary(model1)
```
## Combine PCL scores with 30 days data
```{r pcl_charac}
# characteristics of pcl sample
pclNoNa <- filter(threeMeds_30days, !is.na(BPCLTOT))
...
```{r binomialReg}
# Binomial regression
model2 <- glm(censore30 ~ scale(pclR) + scale(pclA) + scale(pclN) + scale(pclDA) + scale(pclAA) + AGE_OCT01 + PSYCTR + sumComorbid + sumMed, data=pclNoNa, family = binomial)
summary(model2)
anova(model2)
```
...
export_summs(model2, scale = TRUE, error_format = "[{conf.low}, {conf.high}]", digits = 5)
```
```
