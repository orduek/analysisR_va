...
pclNetwork <- filter(pclNetwork, PCL_PHQdiff <= 14 & PCL_PHQdiff >= -14)
...
centralityPlot(list("Without medication" = results_NoMed, "With Medication" = results_Net2), include = c("Betweenness", "Strength", "Closeness"), decreasing = TRUE) + theme_minimal() 
```
...
qgraph(delta, labels = names, layout=L, vsize=6, cut=0, maximum=.5, 
       ...
       groups=gr3,legend.cex=.4, nodeNames = names)
...
compare_12 <- NCT(pclItem_na,pclNoMed_na, it=5000, binary.data=FALSE, test.edges=TRUE, edges='all', progressbar=TRUE)
```

```{r}
compare_12$nwinv.pval
sum(compare_12$einv.pvals$"p-value" < 0.05) # how many edges are less than p=0.05 
...
# 
library(EGAnet)
ega<-EGA(pclItems_noCluster, plot.EGA = TRUE, steps = 4)
ega<-EGA(pclItems_noCluster, plot.EGA = TRUE, steps = 2) # here they find the four clusters like DSM
```
