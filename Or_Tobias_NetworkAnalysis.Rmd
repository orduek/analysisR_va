...
sort(centrality(n2)$InExpectedInfluence  , decreasing = T)
# plot centrality
...
```{r}
# Bootstrap 1:
boot1 <- bootnet(n2, nCores = 7, nBoots = 2000, type = "nonparametric") 
plot(boot1, labels = F, order = "sample")  + theme_minimal()
# now lets look at subjects
boot2 <- bootnet(n2, nCores = 7, nBoots = 2000, type = "case")
plot(boot2, labels = F, order = "sample") + theme_minimal()

# Measure Centrality stability for EI - to check later for differences
boot3 <- bootnet(n2, nCores = 6, nBoots = 2000, statistics = "expectedInfluence", type = "nonparametric")
...
sort(centrality(n2_phq)$InExpectedInfluence, decreasing = T)
# plot centrality
...
```{r}
# Bootstrap 1:
boot1_phq <- bootnet(n2_phq, nCores = 7, nBoots = 2000, type = "nonparametric") 
plot(boot1_phq, labels = F, order = "sample")  + theme_minimal()
# now lets look at subjects
boot2_phq <- bootnet(n2_phq, nCores = 7, nBoots = 2000, type = "case")
plot(boot2_phq, labels = F, order = "sample") + theme_minimal()


# Measure Centrality stability for EI - to check later for differences
boot3_phq <- bootnet(n2_phq, nCores = 10, nBoots = 2000, statistics = "expectedInfluence", type = "nonparametric")
...
pclPhq <- rename(pclPhq,ExpectedInfluence = pclPhq_EI, Predictability = pclPhq_R2_2)
pclPhq$labels_2 <- factor(pclPhq$labels_2, ordered = TRUE, levels = labels_2)
pclPhq_long <- pivot_longer(pclPhq, cols = c(ExpectedInfluence, Predictability))
ggplot(pclPhq_long, aes(x=labels_2, y=value, group = name)) + geom_point() + geom_line() + facet_grid(.~name)+ theme_minimal() + ylab("EI/R^2") + xlab("Node") + coord_flip()


order(pcl_only$labels)
...
cor.test(pclOnly_EI, pclPhq_EI[1:17]) # 0.9

pcl_only_long$group = 'PTSD Only'
pclPhq_long$group = 'PTSD & Depressive Symptoms'
# plot the two toghether. 
ggplot(pcl_only_long, aes(x=labels, y=value, color=group, group=group))  +geom_point()+ geom_line() + geom_point(data=pclPhq_long, aes(x=labels_2, y=value, color=group, group=group))+ geom_line(data=pclPhq_long, aes(x=labels_2, y=value, color=group, group=group)) +facet_grid(.~name) + theme_minimal() + ylab("EI/R^2") + xlab("Node") + coord_flip()
